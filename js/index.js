...
var temperature,Tminimum,Tmaximum ;    //Global Variables

function httpsLocation(){ 
      $(".fa-pulse").removeClass("hidden");
  $(".compass").remove();
  $('#footer').hide();
  $('#location').hide();
  $('#hr1').hide();
  ...
  if (navigator.geolocation){ 
       navigator.geolocation.getCurrentPosition(passLoocation);
    }
}
function passLoocation(position){
...
function fun(obj){    //Toggle between Celcius and Fahrenheit
   if (!obj.checked)
    { 
       $("#temp").html(Math.round(9/5*temperature+32)+"°F");   
       $("#maxTemp").html("<i class='fa fa-angle-up' aria-hidden='true'></i> "+Math.round(9/5*Tmaximum+32)+"°F");   
       $("#minTemp").html("<i class='fa fa-angle-down' aria-hidden='true'></i> "+Math.round(9/5*Tminimum+32)+"°F"); 
       obj.checked=!obj.checked;
    }
    else
    {    
      $("#temp").html(temperature+"°C");   
      $("#maxTemp").html("<i class='fa fa-angle-up' aria-hidden='true'></i> "+Tmaximum+"°C");   
      $("#minTemp").html("<i class='fa fa-angle-down' aria-hidden='true'></i> "+Tminimum+"°C"); 
      obj.checked=!obj.checked;
    }
}